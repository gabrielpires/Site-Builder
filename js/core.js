...
function ChanceIsObject(control, id)
{
	var containerName = '#ctnIsObject_' + id;
	if(control.checked)
	{
		$(containerName).attr('style','display:block');
	}
	else
	{
		$(containerName).attr('style','display:none');
	}

	var hiddenControl = '#hdnIsObject_' + id;
	$(hiddenControl).val(control.checked);
}
